...
- 用量同步由独立的 `usage-sync-worker` 容器负责，实时核验由独立的 `usage-realtime-worker` 容器负责；Web 容器以 `USAGE_SYNC_ROLE=reader` 只读快照，页面访问和手动刷新都不会触发上游查询。刷新队列失败会按 `USAGE_REFRESH_RETRY_BASE_SECONDS` / `USAGE_REFRESH_RETRY_MAX_SECONDS` 退避重试，超过 `USAGE_REFRESH_CLAIM_STALE_SECONDS`（默认 300 秒）的中断 claim 会由 worker 回收。首次启动会回填 `USAGE_INITIAL_BACKFILL_DAYS`（默认 90）天，之后按 `USAGE_SYNC_INTERVAL_SECONDS`（默认 1800 秒）刷新最近 `USAGE_SYNC_RECENT_DAYS`（默认 2）天，并按 `USAGE_SYNC_CALIBRATION_INTERVAL_SECONDS`（默认 21600 秒）补跑最近 `USAGE_SYNC_CALIBRATION_DAYS`（默认 3 天）校准。
...
- 规范工具账号命名使用 `cursor-邮箱前缀`、`claude-code-邮箱前缀`；邮箱前缀只是生成规范别名的输入，不能单独作为账号归属证明。
- Her 是可选补充来源，会独立按其来源内的强身份信息建立账号索引；它不是主来源账号的替代目录。
- 员工不能通过前端传入任意 `user_id` 查询他人数据。

### 双 LiteLLM 来源处理决策

主来源 `primary` 与 Her 来源必须先分开查询、分开验证，再决定是否在聚合或展示层结合。两者不共享账号、权限、令牌或无法区分来源的缓存。

| 场景 | 处理方式 |
| --- | --- |
| 主来源与 Her 的账号解析 | 分开查询、分开验证 |
| 主来源可用、Her 返回 404 或超时 | 返回主来源结果，Her 独立降级并记录安全诊断 |
| 两个来源都找到同一标准化邮箱账号 | 保留来源字段和匹配证明后合并展示 |
| 同一来源下多个规范 `cursor-*` / `claude-code-*` 账号 | 在该来源内合并为同一已验证个人账号范围 |
| 只有姓名或邮箱前缀匹配 | 不得自动绑定 |
| `/key/list` 忽略过滤参数 | 本地重新校验 `key_alias` 后才能接受结果 |
| 账号信息返回空对象、缺少 ID 或结构异常 | 视为该账号不存在或该来源不可用 |
| 一个来源返回 401、403、5xx 或超时 | 不回退到另一个来源，不改写本地映射 |
| 需要写回本地上游映射 | 只使用正确来源的强身份匹配结果，并保留来源字段 |

多来源结合只发生在每个来源都完成独立身份验证之后；一个来源的失败、空结果或账号存在不能证明另一个来源的账号归属。生产排查必须分别验证 `primary` 和 Her，不得只根据 `/api/health` 的总体状态判断某个来源的账号或令牌是否存在。
