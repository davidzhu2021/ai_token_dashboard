...
Docker 镜像内部使用 `8000` 端口，本地建议映射到 `8010`，便于复用 Casdoor 本地回调地址 `http://127.0.0.1:8010/api/auth/callback`。
...
USERS_CACHE_TTL_SECONDS=1800
TEAMS_CACHE_TTL_SECONDS=600
TEAM_MAP_CACHE_TTL_SECONDS=600
...
- `APP_BASE_URL` 本地开发可改为 `http://127.0.0.1:8010`，生产环境使用正式域名。
...
`USERS_CACHE_TTL_SECONDS`、`TEAMS_CACHE_TTL_SECONDS` 和 `TEAM_MAP_CACHE_TTL_SECONDS` 用于缓存上游用户、团队和团队映射列表，可减少登录、个人用量和团队权限检查时的重复分页请求。
