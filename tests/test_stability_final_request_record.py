...
def test_final_request_record_maps_fields() -> None:
    ...
    (backend_id, event_id, request_id, trace_id, attempt_id, attempt_index,
     ...
     event_type, status, error_code, error_class, error_category, scenario,
     scenario_version, event_time, event_date, started_at, ended_at, ttft_ms,
     duration_ms, fallback_from, fallback_to, is_retry, is_fallback,
...
def test_final_request_record_success_no_retry() -> None:
    ...
    assert record[26] is False       # is_retry
...
def test_final_request_record_duration_fallback() -> None:
    ...
    assert record is not None
    assert record[23] == 2500.0  # duration_ms 由 start/end 推导
...
def test_final_request_record_usage_date_override() -> None:
    ...
    assert record is not None
    assert record[19].isoformat() == "2026-08-09"  # event_date 优先取 usage_date（本地日界）
