...
function renderCostOverview() {
  ...
  el("costMetrics").innerHTML = [
    observabilityMetricCard({ label: "当前筛选区间花费及预算", metric: intervalActual, formatter: observabilityMoney, action: "cost-actual-ledger", hint: `区间预算 ${observabilityMoney(metrics.intervalBudget ?? metrics.monthBudget ?? metrics.budget)}${metrics.budgetDelta == null ? "" : ` · 预算差额 ${observabilityMoney(metrics.budgetDelta)}`} · ${data.startDate || ""} ～ ${data.endDate || ""}` }),
    observabilityMetricCard({ label: "年度累计", metric: annualActual, formatter: observabilityMoney, action: "cost-actual-ledger" }),
